...
1. **Wrangler config** is committed as `wrangler.jsonc` (`main`: `.open-next/worker.js`, `assets`: `.open-next/assets`). It sets **`find_additional_modules`** and **`CompiledWasm`** with **`globs: ["**/*.wasm"]`** so Prisma’s loader can **`readAll`** `query_compiler_bg.wasm` under `/bundle/node_modules/.prisma/client/` at runtime. Do **not** narrow that glob to the filename only — deploys can succeed while Prisma still fails with “no such file or directory” for the WASM. Unused **`query_engine_bg.*`** is removed after build by **`scripts/prune-prisma-worker-wasm.mjs`** (see [Wrangler: find additional modules](https://developers.cloudflare.com/workers/wrangler/configuration/#find-additional-modules)).
